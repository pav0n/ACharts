    ...
    public override func drawValues(context context: CGContext?)
    {
        ...
        if (passesCheck())
        {
            ...
            for (var i = 0, count = barData.dataSetCount; i < count; i++)
            {
                ...
                let valueTextColor = UIColor.redColor()
                    ...
                    for (var j = 0, count = Int(ceil(CGFloat(valuePoints.count) * _animator.phaseX)); j < count; j++)
                    {
                        ...
                        else
                        {
                            ...
                            for (var k = 0; k < transformed.count; k++)
                            {
                                ...
                                if (k==0){
                                    print("FORMATTER_2: \(formatter)")
                                    print("YPos\(y+CGFloat(10))")
                                    print("Value_2:\(formatter!.stringFromNumber(vals[k])!)")
                                    print("K:\(k)")
